...
export async function getDeezerId(id) {
  ...
  const response = await fetch(`https://api.deezer.com/search/artist?q=artist:"${name}"&strict=on`)

  const data = await response.json()

  if (!data) return {}

  const filteredArtists = data.data.filter((value) => name.toLowerCase() === value.name.toLowerCase())
  
  return filteredArtists[0]?.id
}
